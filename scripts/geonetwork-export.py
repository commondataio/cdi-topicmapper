...
def run():
  ...
  for record in coll.find():
    ...
    if 'topicCat' in record.keys():
      if isinstance(record['topicCat'], str):
        k_list = [record['topicCat']]
      else:
        k_list = record['topicCat']
      for k in k_list:
        v = topics.get(k, 0)
  ...
  dict2csv(topics, ['name', 'value'], '../data/geonetwork_topics.csv')
  dict2csv(keywords, ['name', 'value'], '../data/geonetwork_keywords.csv')
